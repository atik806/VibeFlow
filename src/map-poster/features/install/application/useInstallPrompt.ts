...
import { isNativePlatform } from "@/map-poster/core/platform";
import {
...
declare global {
  interface Window {
    __vfDeferredInstallPrompt?: Event;
  }
}
...
export default function useInstallPrompt() {
  ...
  useEffect(() => {
    ...
    setBeforeInstallPromptFired(Boolean(window.__vfDeferredInstallPrompt));
    ...
    if (window.__vfDeferredInstallPrompt) {
      setDeferredPrompt(
        window.__vfDeferredInstallPrompt as BeforeInstallPromptEvent,
      );
    }
    ...
    const handler = (e: Event) => {
      ...
      window.__vfDeferredInstallPrompt = e;
    };
    window.addEventListener("beforeinstallprompt", handler);

    const fallbackTimer = window.setTimeout(() => {
      if (isAndroid() && !window.__vfDeferredInstallPrompt) {
  ...
  const diagnostics = {
    ...
    deferredPromptCached: Boolean(window.__vfDeferredInstallPrompt),
